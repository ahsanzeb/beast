...
\rev{
consider a few diff $d^{n}$ cases:
1 e: for a given qa,qb; there might be incentives to prefer one angle over the other....
but more interesting cases:
where one angle does not lower the energy of many-electron ground state
while the other does. so, obviously, such systems would take the energetically favorable path...
}
...
and find significant splitting in these levels
...
determines the sign and strength of this effect
...
that depends on which ion cage dominates.
