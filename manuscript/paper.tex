...
we can express their (crystal field) potential in terms of spherical harmonics~\cite{PavariniChap}, 
\begin{align}
...
\label{sec:egt2g}
Consider six oxygen (O) ions each with a charge $-q_o$ making an octahedron of size $a$ (placed at the face centres of a cube of edge length $a$).
...
still contains only a few non-zero components.
Not only the ratio 
$V_{4,0}^{JT}:V_{4,4}^{JT}$ now deviates from
 ideal 
octahedral field as in Eq.~\ref{eq:voct},
...
$t_{2g}$ and $e_{g}$ manifolds due to the deformations produced 
in the crystal field as the octahedra rotate or tilt.
...
We will see that these with the octahedral rotations and tilting, 
these potentials rotate and deform.
...
so we consider the octahedron $1$ with central B ion at $\text{a}/2$
...
$t_{2g}$ spectrum computed using the potential of full lattice compared with that obtained from only A,B cages (and the anion octahedron itself),
shown as a function of $q_A/q_o$ at 
$\alpha=0,\beta=20^\circ$, $\Delta_o=4q_o/15a$.
Full lattice only shifts the balance between the two cation cages towards A but keeps the qualitative picture the same.
}
