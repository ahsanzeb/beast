...
$V_{l,m}$ of a potentials $V(\vec r)$ %of $A/B$ ion cages 
at a position $\vec r$ around a JT ion.
Let's consider $V^{A}(\vec r)$ to illustrate the idea. 
$V^{B}(\vec r)$ and $V^{JT}(\vec r)$ can also be dealt the same way.
...
taken out the factor $\frac{q_A}{a}$ for clarity.
It is worth noting that $V_{l,m}^{A}$ can be found without 
evaluating the 
inner products between the spherical harmonics and
$V^{A}(\vec r)=V^{A}(\vec r;\alpha,\beta)$, as follows.
...
This gives a set of
linear equations in $\{V_{l,m}^{A}\}$ (for a fixed value of $l$)
