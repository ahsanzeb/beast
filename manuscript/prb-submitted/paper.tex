...
Unlike the electronic correlations and spin-orbit interaction,
 the hybridization between 
the orbitals localized on different B sites are not
...
and
the hybridization between the orbitals localized on different B sites
(and the electronic band structure arising from them)
are modified as the octahedra rotate.
...
using the Slater-Koster method~\cite{SlaterKoster}
and
calculate the change in the energy of two configurations
$t_{2g}^1$ and $e_g^1$
as the octahedra rotate
in three cases: with either the crystal field or the hopping only and with both of these simultaneously.
...
We use Slater-Koster hopping parameters
...
In contrast, the energy of $t_{2g}^1$ decreases due band widening arising from 
...
and thus reduces the bandwidths and
affects the associated rotation induced energetic changes.
...
\caption{The effect of dispersion due to the electron hopping between B sites.
