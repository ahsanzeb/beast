...
def getxy(iu,isp,spin):
	...
	ranges=[[range(6,11),range(22,27)],[range(3,6),range(19,22)]]
	# select range of columns
	rang = ranges[isp-1][spin];
...
for iu in range(1,6,1):
	...
	for spin in [0,1]:
		...
		if iu==1 and spin==1:
			...
			ax.plot(x,y1,lw=3,color="red",label="Ni$_{d}$")
			ax.plot(x,y2,lw=3,color="blue",label="O$_{p}$")
		else:
